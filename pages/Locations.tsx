    ...
    <div className="space-y-4 pb-20">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-[10px] text-slate-500 font-black uppercase tracking-widest mt-1">Ubicaciones</p>
        </div>
      ...
      <div className="relative">
        ...
        <input type="text" className="w-full pl-10 pr-10 py-2.5 bg-white border border-slate-100 rounded-xl text-xs outline-none shadow-sm focus:ring-2 focus:ring-indigo-500" placeholder="Buscar almacén..." value={search} onChange={e => setSearch(e.target.value)} />
      </div>
        ...
        {loading ? (
          ...
          <div key={l.id} className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex items-center justify-between group">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-slate-50 rounded-lg text-slate-400 group-hover:bg-indigo-50 group-hover:text-indigo-600 transition-colors"><Warehouse className="w-4 h-4" /></div>
              <div>
                <p className="text-xs font-bold text-slate-800 uppercase tracking-tight">{l.name}</p>
              </div>
            </div>
            <div className="flex gap-1">
              <button onClick={() => handleOpenModal(l)} className="p-1.5 text-slate-300 hover:text-indigo-600"><Edit2 className="w-3.5 h-3.5" /></button>
              <button onClick={() => handleDelete(l.id)} className="p-1.5 text-slate-300 hover:text-rose-600"><Trash2 className="w-3.5 h-3.5" /></button>
            </div>
          </div>
        ))}
      </div>
