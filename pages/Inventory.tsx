    ...
    <div className="space-y-4 animate-in fade-in pb-24">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
        <div>
          ...
          <p className="text-[9px] text-slate-500 font-black uppercase tracking-widest mt-0.5">Inventario Maestro</p>
        </div>
        <div className="flex gap-2 w-full sm:w-auto">
          <div className="bg-white border border-slate-200 rounded-xl flex overflow-hidden shadow-sm flex-1 sm:flex-none">
            <button onClick={handleExportPDF} className="flex-1 px-4 py-3 text-slate-600 text-[9px] font-black uppercase flex items-center justify-center gap-1.5 hover:bg-slate-50 transition-all border-r border-slate-100"><FileText className="w-3.5 h-3.5" /> PDF</button>
            <button onClick={() => exportToExcel(products, "Inventario", "Stock")} className="flex-1 px-4 py-3 text-emerald-600 text-[9px] font-black uppercase flex items-center justify-center gap-1.5 hover:bg-emerald-50 transition-all"><FileSpreadsheet className="w-3.5 h-3.5" /> EXCEL</button>
          </div>
          ...
          <thead className="bg-slate-50/50 text-[8px] font-black uppercase text-slate-400 tracking-widest border-b">
            <tr>
              <th className="px-6 py-4 text-left w-10">
                <button onClick={toggleSelectAll} className="p-1 hover:bg-slate-200 rounded-lg transition-colors">
                  {selectedIds.length === filtered.length && filtered.length > 0 ? <CheckSquare className="w-4 h-4 text-indigo-600" /> : <Square className="w-4 h-4" />}
                </button>
              </th>
            ...
            {filtered.map(p => (
              <tr key={p.id} className={`transition-colors group ${selectedIds.includes(p.id) ? 'bg-indigo-50/30' : 'hover:bg-slate-50/40'}`}>
                <td className="px-6 py-3">
                  <button onClick={() => toggleSelect(p.id)} className="p-1 hover:bg-slate-200 rounded-lg transition-colors">
          ...
          <form onSubmit={handleSubmit} className="relative bg-white w-full max-w-4xl rounded-[3rem] overflow-hidden flex flex-col shadow-2xl animate-in zoom-in-95 max-h-[92vh]">
            ...
            <div className="overflow-y-auto p-8 space-y-8 no-scrollbar">
               <div className="grid grid-cols-1 md:grid-cols-12 gap-10">
                  ...
                  <div className="md:col-span-8 grid grid-cols-1 sm:grid-cols-2 gap-5">
                    ...
                    <div className="space-y-1.5">
                      ...
                      <input type="text" placeholder="AUTO-GENERADO" className="w-full p-4 bg-slate-100 rounded-2xl outline-none font-bold text-xs uppercase border-2 border-transparent focus:border-indigo-500 transition-all" value={formData.code} onChange={e => setFormData({...formData, code: e.target.value.toUpperCase()})} />
                    </div>

                    <div className="space-y-1.5">
                      <label className="text-[9px] font-black text-slate-400 uppercase ml-2 tracking-widest">Marca</label>
                      <input type="text" placeholder="MARCA..." className="w-full p-4 bg-slate-100 rounded-2xl outline-none font-bold text-xs uppercase border-2 border-transparent focus:border-indigo-500 transition-all" value={formData.brand} onChange={e => setFormData({...formData, brand: e.target.value})} />
                    </div>

                    <div className="space-y-1.5">
                      <label className="text-[9px] font-black text-slate-400 uppercase ml-2 tracking-widest">Modelo</label>
                      <input type="text" placeholder="MODELO..." className="w-full p-4 bg-slate-100 rounded-2xl outline-none font-bold text-xs uppercase border-2 border-transparent focus:border-indigo-500 transition-all" value={formData.model} onChange={e => setFormData({...formData, model: e.target.value})} />
                    </div>

                    <div className="space-y-1.5">
                      <label className="text-[9px] font-black text-slate-400 uppercase ml-2 tracking-widest">Unidad</label>
                      <select className="w-full p-4 bg-slate-100 rounded-2xl font-bold text-xs uppercase outline-none border-2 border-transparent focus:border-indigo-500 transition-all" value={formData.unit} onChange={e => setFormData({...formData, unit: e.target.value})}>
               ...
               <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                  <div className="space-y-1.5">
                    <label className="text-[9px] font-black text-slate-400 uppercase ml-2 tracking-widest">Almacén</label>
                    <select required className="w-full p-4 bg-slate-100 rounded-2xl font-bold text-xs uppercase border-2 border-transparent focus:border-indigo-500 transition-all" value={formData.location} onChange={e => setFormData({...formData, location: e.target.value})}>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <select required className="w-full p-4 bg-slate-100 rounded-2xl font-bold text-xs uppercase border-2 border-transparent focus:border-indigo-500 transition-all" value={formData.category} onChange={e => setFormData({...formData, category: e.target.value})}>
                        <option value="" disabled>Seleccione...</option>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input type="text" className="w-full p-4 bg-slate-100 rounded-2xl outline-none font-bold text-xs uppercase border-2 border-transparent focus:border-indigo-500 transition-all" value={formData.size} onChange={e => setFormData({...formData, size: e.target.value})} />
                  </div>
               </div>
               ...
               <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 bg-slate-900 p-8 rounded-[2.5rem] shadow-2xl">
                  <div className="text-center space-y-2">
                    ...
                    <input type="number" className="w-full p-4 bg-white/10 rounded-2xl text-center font-black text-xl text-white outline-none border-2 border-white/5 focus:border-indigo-500 transition-all" value={formData.stock || ''} placeholder="0" onChange={e => setFormData({...formData, stock: Number(e.target.value)})} disabled={!!editingProduct} />
                  </div>
                  <div className="text-center space-y-2">
                    <label className="text-[9px] font-black text-rose-400 uppercase tracking-widest">Alerta Stock Bajo</label>
                    <input type="number" className="w-full p-4 bg-white/10 rounded-2xl text-center font-black text-xl text-rose-400 outline-none border-2 border-white/5 focus:border-rose-500 transition-all" value={formData.minStock || ''} placeholder="30" onChange={e => setFormData({...formData, minStock: Number(e.target.value)})} />
                  </div>
                  <div className="text-center space-y-2">
                    <label className="text-[9px] font-black text-emerald-400 uppercase tracking-widest">Costo Unitario</label>
                    <input type="number" step="0.01" className="w-full p-4 bg-white/10 rounded-2xl text-center font-black text-xl text-emerald-400 outline-none border-2 border-white/5 focus:border-emerald-500 transition-all" value={formData.purchasePrice || ''} placeholder="0.00" onChange={e => setFormData({...formData, purchasePrice: Number(e.target.value)})} />
                  </div>
               </div>
            </div>
            ...
            <div className="px-10 py-6 border-t bg-white flex gap-5 shrink-0">
               ...
               <button type="submit" disabled={saving} className="flex-[3] py-4 bg-indigo-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-2xl flex items-center justify-center gap-2 hover:bg-indigo-700 active:scale-95 disabled:opacity-50">
                  {saving ? <Loader2 className="animate-spin w-5 h-5" /> : <Save className="w-5 h-5" />} Confirmar Cambios
               </button>
            </div>
          </form>
        </div>
