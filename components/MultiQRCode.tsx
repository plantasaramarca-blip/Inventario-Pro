      ...
      <div className="relative bg-white rounded-[3rem] w-full max-w-5xl h-[90vh] flex flex-col shadow-2xl animate-in zoom-in-95">
        <div className="p-8 border-b flex justify-between items-center bg-slate-50/50">
          ...
          <div className="flex gap-3">
             <button onClick={handleDownload} disabled={generating} className="bg-indigo-600 text-white px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest flex items-center gap-2 shadow-xl hover:bg-indigo-700 disabled:opacity-50">
        ...
        <div className="flex-1 overflow-y-auto p-10 bg-slate-200/50 no-scrollbar">
           {/* Contenedor Planilla A4 (Simulada) */}
           <div 
             ref={containerRef}
             className="bg-white mx-auto shadow-2xl p-8 grid grid-cols-3 gap-6"
             style={{ width: '210mm', minHeight: '297mm' }}
           >
             {products.map(p => (
               <div key={p.id} className="border border-slate-900 flex flex-col items-center justify-center p-4 h-[40mm]">
                  <QRCodeSVG value={`${window.location.origin}?id=${p.id}`} size={70} />
                  <p className="mt-2 text-[10px] font-black uppercase text-center truncate w-full">{p.name}</p>
                  <p className="text-[12px] font-black text-center mt-1">{p.code}</p>
               </div>
             ))}
           </div>
        </div>
