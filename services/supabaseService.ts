...
export const getMovements = async (): Promise<Movement[]> => {
  ...
  return (data || []).map(m => ({ 
    ...
    balanceAfter: m.balance_after, destinationName: m.destino_nombre 
  }));
};
...
export const saveContact = async (contact: Partial<Contact>) => {
  ...
  const { error } = contact.id ? await supabase.from('contacts').update(payload).eq('id', contact.id) : await supabase.from('contacts').insert([payload]);
  if (error) throw error;
};

export const deleteContact = async (id: string) => { if (useSupabase()) await supabase.from('contacts').delete().eq('id', id); };
...
export const getAuditLogs = async (p=0, l=50) => { 
  ...
  const { data, count } = await supabase.from('audit_logs').select('*', { count: 'exact' }).order('created_at', { ascending: false }).range(p*l, (p+1)*l-1); 
